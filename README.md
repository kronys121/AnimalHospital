...
Что получается: уличный вход с лобби (стулья, точка спавна пациента), увеличенный
ресепшн со стойкой, коридор, четыре кабинета (Basic Medical / DNA, X-Ray,
Heart Monitor, Surgery) и комната отдыха под будущий магазин. Пациент появляется
в лобби у входа, проходит через зал ожидания в ресепшн и там встаёт в очередь
к стойке. У каждой комнаты одинаковая структура: `Structure`, `EntryPoint`,
`InteractionZone`, `RoomLabel`. Точные координаты и дерево инстансов описаны
в `docs/stage-0-layout.md`.
